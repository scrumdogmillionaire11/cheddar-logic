-- DEV cleanup for suspected rogue card_display_log rows.
--
-- Scope: delete HIGH-confidence suspicious rows that are already settled,
-- so they can no longer render on /results.
--
-- Safety:
-- 1) takes a backup snapshot table before delete
-- 2) transaction-wrapped
-- 3) no-op if candidate set is empty
--
-- Usage:
--   sqlite3 "$CHEDDAR_DB_PATH" < scripts/sql/cleanup_card_display_log_backfill_dev.sql
...
-- Backup only rows about to be deleted.
DROP TABLE IF EXISTS card_display_log_cleanup_backup_dev;
CREATE TABLE card_display_log_cleanup_backup_dev AS
SELECT *
...
-- Post-check: candidates remaining should be 0.
SELECT
  COUNT(*) AS remaining_candidates
FROM card_display_log cdl
INNER JOIN _cleanup_candidates cc ON cc.id = cdl.id;

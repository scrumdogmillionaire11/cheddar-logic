...
Output: {phase}-UAT.md tracking all test results. If issues found: diagnosed gaps, verified fix plans ready for /pax:execute-phase
</objective>

<execution_context>
@./.claude/process-acceleration-executors/workflows/verify-work.md
@./.claude/process-acceleration-executors/templates/UAT.md
</execution_context>
...
8. If issues found:
   ...
   - Spawn pax-planner in --gaps mode to create fix plans
   - Spawn pax-plan-checker to verify fix plans
   - Iterate planner ↔ checker until plans pass (max 3)
   - Present ready status with `/clear` then `/pax:execute-phase`
</process>
...
- /pax:execute-phase {Z+1} — skip to execution (if already planned)
...
**Also available:**
- /pax:complete-milestone — skip audit, archive directly
...
- /pax:plan-phase {Z} --gaps — regenerate fix plans
...
**Options:**
- /pax:plan-phase {Z} --gaps — retry fix planning with guidance
- /pax:discuss-phase {Z} — gather more context before replanning
...
- [ ] If issues: pax-planner creates fix plans from diagnosed gaps
- [ ] If issues: pax-plan-checker verifies fix plans (max 3 iterations)
- [ ] Ready for `/pax:execute-phase` when complete
</success_criteria>

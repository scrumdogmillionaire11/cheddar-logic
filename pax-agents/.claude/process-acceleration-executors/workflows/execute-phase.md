...
**No dependency analysis needed.** Wave numbers are pre-computed during `/pax:plan-phase`.
...
2. **Read files and spawn all autonomous agents in wave simultaneously:**
   ...
   @./.claude/process-acceleration-executors/references/tdd.md
   </execution_context>
...
1. **Spawn agent for checkpoint plan:**
   ```
   Task(prompt="{subagent-task-prompt}", subagent_type="pax-executor", model="{executor_model}")
   ```
...
| `gaps_found` | Present gap summary, offer `/pax:plan-phase {phase} --gaps` |
...
- `/pax:verify-work {X}` — manual testing before planning
```

User runs `/pax:plan-phase {X} --gaps` which:
1. Reads VERIFICATION.md gaps
2. Creates additional plans (04, 05, etc.) with `gap_closure: true` to close gaps
3. User then runs `/pax:execute-phase {X} --gaps-only`
...
1. Run `/pax:execute-phase {phase}` again

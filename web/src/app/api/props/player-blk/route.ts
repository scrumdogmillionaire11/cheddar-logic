/**
 * GET /api/props/player-blk
 *
 * Returns recent player_blk_logs rows across all tracked players.
 *
 * Query params:
...
export async function GET(request: NextRequest) {
  ...
  try {
    const securityCheck = performSecurityChecks(request, '/api/props/player-blk');
    ...
    let rows: PlayerBlkRow[];
    if (dateParam) {
      rows = db
        .prepare(
          `SELECT id, sport, player_id, player_name, game_id, game_date,
                  opponent, is_home, blocked_shots, toi_minutes, fetched_at, created_at
           FROM player_blk_logs
           WHERE game_date = ?
           ORDER BY player_name ASC, fetched_at DESC
           LIMIT ?`,
        )
        .all(dateParam, limit) as PlayerBlkRow[];
    } else {
      rows = db
        .prepare(
          `SELECT id, sport, player_id, player_name, game_id, game_date,

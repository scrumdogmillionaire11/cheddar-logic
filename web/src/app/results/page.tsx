...
function resultBadgeClass(result: "WIN" | "LOSS" | "PUSH" | "PENDING") {
  if (result === "WIN") return "border-emerald-500/40 bg-emerald-500/15 text-emerald-200";
  if (result === "LOSS") return "border-rose-500/40 bg-rose-500/15 text-rose-200";
  if (result === "PUSH") return "border-amber-500/40 bg-amber-500/15 text-amber-200";
  return "border-white/20 bg-white/5 text-cloud/60";
}

function roiTextClass(value: number | null | undefined) {
  if (value === null || value === undefined || Number.isNaN(value)) return "text-cloud/70";
...
export default function ResultsPage() {
  ...
  const loadResults = useCallback(async () => {
    try {
      ...
      if (includeOrphanedValue !== null) params.set('include_orphaned', includeOrphanedValue);
  ...
  }, [filterSport, filterCategory, filterHighConf, filterMarket, includeOrphanedValue, dedupeValue]);
      ...
      <div className="relative mx-auto max-w-6xl px-6 py-12">
        ...
        <header className="mb-10 space-y-3">
          ...
          {!loading && dataMeta ? (
            <p className="text-xs text-cloud/50">
              Dev view: include orphaned {dataMeta.includeOrphaned ? 'on' : 'off'} · dedupe {dataMeta.dedupe ? 'on' : 'off'} · returned {dataMeta.returnedCount} / filtered {dataMeta.filteredCount} / settled {dataMeta.totalSettled}
            </p>
          ) : null}
        </header>
        ...
        <section className="sticky top-0 z-10 mb-6 rounded-xl border border-white/10 bg-night/85 px-4 py-3 backdrop-blur md:hidden">
          <div className="flex items-center justify-between text-sm">
            ...
            <span className={`font-semibold ${roiTextClass(summary?.totalPnlUnits)}`}>
                ...
                {segments.map((row) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={roiTextClass(row.totalPnlUnits)}>{formatUnits(row.totalPnlUnits)}</span>
          ...
          <div className="mt-6 space-y-3 md:hidden">
            {segments.length === 0 ? (
              <div className="rounded-xl border border-white/10 bg-night/30 px-4 py-6 text-sm text-cloud/60">
                No graded segments yet. This list populates automatically once plays are graded.
              </div>
            ) : (
              segments.map((row) => {
                const total = row.wins + row.losses + row.pushes;
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="text-sm font-semibold text-cloud">
                          {row.sport} - <span className="capitalize text-cloud/75">{row.cardCategory}</span>
                        </p>
                        <p className="mt-1 text-xs text-cloud/60 capitalize">
                          {row.recommendedBetType || "--"}
                        </p>
                      </div>
                      <span className={`text-sm font-semibold ${roiTextClass(row.totalPnlUnits)}`}>
                        {formatUnits(row.totalPnlUnits)}
                      </span>
                    </div>
                    <div className="mt-3 grid grid-cols-3 gap-2">
                      <div className="rounded-lg border border-white/10 bg-white/5 px-2 py-2 text-center">
                        <p className="text-base font-semibold text-cloud">{total}</p>
                        <p className="text-[11px] text-cloud/50">Plays</p>
                      </div>
                      <div className="rounded-lg border border-white/10 bg-white/5 px-2 py-2 text-center">
                        <p className="text-base font-semibold text-cloud">{formatPercent(winRate)}</p>
                        <p className="text-[11px] text-cloud/50">Win Rate</p>
                      </div>
                      <div className="rounded-lg border border-white/10 bg-white/5 px-2 py-2 text-center">
                        <p className={`text-base font-semibold ${roiTextClass(row.totalPnlUnits)}`}>
                          {formatUnits(row.totalPnlUnits)}
                        </p>
                ...
                {visibleLedger.map((row) => {
                  ...
                  return (
                    <div key={row.id} className="grid grid-cols-8 gap-4 px-4 py-3 text-sm text-cloud/70">
                      ...
                      <span>
                        <span className={`rounded-full border px-2 py-1 text-xs font-semibold ${resultBadgeClass(outcome)}`}>
                          {outcome}
                        </span>
                      </span>
                    </div>
                  ...
                  <details key={`${row.id}-mobile`} className="rounded-xl border border-white/10 bg-night/35">
                    <summary className="cursor-pointer list-none px-4 py-4">
                      <div className="flex items-center justify-between gap-3">
                        <p className="text-xs text-cloud/55">
                          {formatLedgerDate(row.settledAt || row.createdAt)} · {row.sport}
                        </p>
                        <span className={`rounded-full border px-2 py-1 text-[11px] font-semibold ${resultBadgeClass(outcome)}`}>
                          {outcome}
                        </span>
                      </div>
                      <p className="mt-1 text-sm font-semibold text-cloud">{matchup}</p>
                      <p className="mt-1 text-sm text-cloud/75">{predictionLine}</p>
                      <div className="mt-2 flex items-center gap-3 text-xs">
                        ...
                        <span className={roiTextClass(row.pnlUnits)}>{formatUnits(row.pnlUnits)}</span>
                      </div>
                    </summary>
                    <div className="grid grid-cols-2 gap-3 border-t border-white/10 px-4 py-3 text-xs">
                      ...
                      <div>
                        ...
                        <p className="mt-0.5 text-cloud/80">{formatPrice(row.price)}</p>
                      </div>
                      <div>
                        <p className="text-cloud/50">Card Type</p>
                        <p className="mt-0.5 text-cloud/80">{formatLabel(row.cardType)}</p>
                      </div>
                    </div>
                  </details>
                );

...
async function run() {
  ...
  const spreadCard1 = buildSpreadCard({
    ...
    drivers: [
      buildDriver({ key: 'spread_driver', market: 'SPREAD', tier: 'BEST', direction: 'HOME', confidence: 0.8, note: 'Strong spread edge', cardTitle: 'Spread Edge' }),
    ],
  });

  const sc1 = getCardDecisionModel(spreadCard1, spreadOdds);
  assert.ok(sc1.spreadCompare !== null, 'Test 1: spreadCompare should be non-null for SPREAD market');
  assert.strictEqual(sc1.spreadCompare.direction, 'HOME', 'Test 1: direction should be HOME');
  assert.strictEqual(sc1.spreadCompare.marketLine, -9.5, 'Test 1: marketLine should be -9.5 (spreadHome)');
  ...
  const spreadCard2 = buildSpreadCard({
    ...
    drivers: [
      buildDriver({ key: 'spread_proj', market: 'SPREAD', tier: 'BEST', direction: 'HOME', confidence: 0.8, note: 'Proj: -8.2 vs market -9.5', cardTitle: 'Spread Projection' }),
    ],
  });

  const sc2 = getCardDecisionModel(spreadCard2, spreadOdds);
  assert.ok(sc2.spreadCompare !== null, 'Test 2: spreadCompare should be non-null');
  assert.strictEqual(sc2.spreadCompare.projectedSpread, -8.2, 'Test 2: projectedSpread should parse -8.2 from note');
  ...
  const spreadCard3 = buildSpreadCard({
    ...
    drivers: [
      buildDriver({ key: 'spread_no_proj', market: 'SPREAD', tier: 'BEST', direction: 'HOME', confidence: 0.8, note: 'Model edge on spread', cardTitle: 'Spread Edge' }),
    ],
  });

  const sc3 = getCardDecisionModel(spreadCard3, spreadOdds);
  assert.ok(sc3.spreadCompare !== null, 'Test 3: spreadCompare should be non-null');
  assert.strictEqual(sc3.spreadCompare.projectedSpread, null, 'Test 3: projectedSpread should be null when no pattern match');
  assert.strictEqual(sc3.spreadCompare.marketLine, -9.5, 'Test 3: marketLine should still be set');
  ...
  const mlCard = buildSpreadCard({
    ...
    drivers: [
      buildDriver({ key: 'ml_driver', market: 'ML', tier: 'BEST', direction: 'HOME', confidence: 0.8, note: 'ML edge', cardTitle: 'ML Edge' }),
    ],
  });

  const sc4 = getCardDecisionModel(mlCard, spreadOdds);
  assert.strictEqual(sc4.spreadCompare, null, 'Test 4: spreadCompare should be null for ML market');
  ...
  const spreadCard5 = buildSpreadCard({
    ...
    drivers: [
      buildDriver({ key: 'spread_away', market: 'SPREAD', tier: 'BEST', direction: 'AWAY', confidence: 0.8, note: 'Away spread edge', cardTitle: 'Away Spread' }),
    ],
  });

  const sc5 = getCardDecisionModel(spreadCard5, spreadOdds);
  assert.ok(sc5.spreadCompare !== null, 'Test 5: spreadCompare should be non-null for AWAY SPREAD');
  assert.strictEqual(sc5.spreadCompare.direction, 'AWAY', 'Test 5: direction should be AWAY');
  assert.strictEqual(sc5.spreadCompare.marketLine, 9.5, 'Test 5: marketLine should be 9.5 (spreadAway)');

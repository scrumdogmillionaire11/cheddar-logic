...
- **Driver mismatch** - totals drivers (PDO, pace) should not influence side picks
- **Math confusion** - using [0, 1] scores that represent probability vs directional push
- **No driver contradiction model** - cannot express when drivers disagree
...
**Key change:** `signal` is signed [-1, +1], not a probability [0, 1]. Use `eligible` to gate whether a driver contributes.
...
- `rest` - lightly weighted (affects pace + defensive execution, about 0.08 weight)
...
- **Risk-only:** `pace`, `pdoRegression` - variance overlay (signal=0, ineligible for direction)
...
- `powerRating` - Team strength to win prob
...
1) Build candidates for this market
   Example: TOTAL -> {side: "OVER"}, {side: "UNDER"}
   Example: SPREAD -> {side: "HOME"}, {side: "AWAY"}
...
2) For each candidate:
   ...
   c) net = sum(signal * weight) over eligible
      net in [-1, +1]
   ...
   e) Compute penalties (market-specific):
      ...
      sum_penalties in [0, 1.0]

   f) score = net - sum_penalties
      score typically in [-0.4, +0.6], depends on penalty tuning
...
3) Choose best candidate only if |net| >= t_dir
   (if net is too close to zero, cannot pick; emit PASS instead)
...
- Penalties:
  ...
  - if in coinflip zone (45% to 55% implied) AND edge exists: penalty = 0
...
IF abs(SPREAD.score - ML.score) <= 0.05
  ...
  AND ML odds are in coinflip zone (45% to 55% implied) or better (plus)
  AND ML.edge > 0
...
**Key:** Never compare edge units directly (points vs percent). Use score for cross-market, apply value realism only for ML vs Spread tie-breaker.

---

## Level 8: Welcome Home Fade (Mechanical, Not Override)

Welcome Home Fade is an injected driver in SPREAD and ML engines only, not a selector override:
...
✅ **Backward compatible** (output shape does not break queries)  

...
- Verification blocker compatibility:
  ...
  - legacy `EDGE_VERIFICATION_REQUIRED` is **sunset** — it must not be emitted by any pipeline path; persisted records containing it are rendered via backward-compat label only

...
| --- | --- | --- |
| `job:refresh-game-lines` | Pull odds → team metrics → MLB pitcher stats + weather → run NBA / NHL / MLB models | You want a fully refreshed slate of spread/ML/total cards |
| `job:refresh-player-props` | Pull NHL shots prop lines → sync player availability → run NHL shots model | You want fresh player prop cards only (⚠️ SOG prop API disabled on 500-token budget) |
| `job:refresh-all` | Both of the above in sequence | Full refresh before a game window |

```bash
# Full refresh — game lines + player props (all sports)
set -a; source .env; set +a; npm --prefix apps/worker run job:refresh-all

# Game lines only (spread / ML / total for NBA, NHL, MLB)
# Soccer disabled — odds fetch off (budget). Also generates game props cards (1P totals, F5/halves).
 set -a; source .env; set +a; npm --prefix apps/worker run job:refresh-game-lines

# Player props only (NHL shots on goal — SOG API disabled on 500-token budget; runs model on cached lines)
set -a; source .env; set +a; npm --prefix apps/worker run job:refresh-player-props
```

> **Game Props note:** First period totals (NHL), F5 first-half lines (MLB), and similar sub-game markets are generated by the same model runs as game lines — there is no separate job. They appear under the **Game Props** filter tab in the UI automatically.

> **NCAAM:** Shut down for off-season. Re-enable with `ENABLE_NCAAM_MODEL=true` before running `job:run-ncaam-model` manually.
...
# Pull NHL player shots props
# ⚠️  DISABLED on 500-token budget — re-enable when on paid API tier
# set -a; source .env; set +a; npm --prefix apps/worker run job:pull-nhl-player-shots-props

# Sync NHL player availability before prop model run (ensures injured players are filtered)
npm --prefix apps/worker run job:sync-nhl-player-availability

# Run NHL player shots prop model (runs on cached lines)
npm --prefix apps/worker run job:run-nhl-player-shots-model
...
npm --prefix apps/worker run job:run-nhl-player-shots-model
set -a; source .env; set +a; npm --prefix apps/worker run job:run-mlb-model
# set -a; source .env; set +a; npm --prefix apps/worker run job:run-ncaam-model  # DISABLED: off-season + budget
# set -a; source .env; set +a; npm --prefix apps/worker run job:run-soccer-model  # DISABLED: odds fetch off (budget)
...
# CHEDDAR_DB_PATH=/opt/data/cheddar-prod.db npm --prefix apps/worker run job:run-ncaam-model  # DISABLED: off-season + budget
# CHEDDAR_DB_PATH=/opt/data/cheddar-prod.db npm --prefix apps/worker run job:run-soccer-model  # DISABLED: odds fetch off (budget)
...
# npm --prefix apps/worker run job:run-ncaam-model  # DISABLED: off-season + budget
# npm --prefix apps/worker run job:run-soccer-model  # DISABLED: odds fetch off (budget)

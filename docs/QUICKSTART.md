...
| `job:refresh-player-props` | Sync player availability → run NHL shots model → run soccer props refresh | You want fresh projection-only prop cards without event-level odds pulls |
...
# Soccer disabled — odds fetch off (budget). Also generates projection-only game props cards (NHL 1P, MLB F5).
 set -a; source .env; set +a; npm --prefix apps/worker run job:refresh-game-lines

# Player props only (projection-only NHL shots + soccer props)
set -a; source .env; set +a; npm --prefix apps/worker run job:refresh-player-props
```

> **Game Props note:** First period totals (NHL), MLB F5, and similar sub-game markets are generated by the same model runs as projection-only lanes — there is no separate odds-pull job.

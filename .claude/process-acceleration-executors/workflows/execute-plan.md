...
@./.claude/process-acceleration-executors/references/git-integration.md
</required_reading>
...
2. Use Task tool with subagent_type="pax-executor" and model="{executor_model}":
...
2. For each segment in order:
   ...
   B. If routing = Subagent:
      ```
      Spawn Task tool with subagent_type="pax-executor" and model="{executor_model}":
...
See `./.claude/process-acceleration-executors/references/tdd.md` for TDD plan structure.
</tdd_plan_execution>
...
See ./.claude/process-acceleration-executors/references/checkpoints.md for complete checkpoint guidance.
</step>
...
Create `.planning/phases/XX-name/{phase}-USER-SETUP.md` using template from `./.claude/process-acceleration-executors/templates/user-setup.md`.
...
Use ./.claude/process-acceleration-executors/templates/summary.md for structure.
...
**Also available:**
- `/pax:verify-work {phase}-{plan}` — manual acceptance testing before continuing
...
- `/pax:discuss-phase {Z+1}` — gather context first
...
**Also available:**
- `/pax:verify-work` — manual acceptance testing before completing milestone
- `/pax:add-phase <description>` — add another phase before completing

...
> **Note:** Planning methodology is in `agents/pax-planner.md`.
...
@./.claude/process-acceleration-executors/references/checkpoints.md
</execution_context>
...
<!-- For checkpoint task examples and patterns, see @./.claude/process-acceleration-executors/references/checkpoints.md -->
...
**Wave is pre-computed:** Wave numbers are assigned during `/pax:plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
See `./.claude/process-acceleration-executors/templates/user-setup.md` for full schema and examples
...
See `./.claude/process-acceleration-executors/workflows/verify-phase.md` for verification logic.

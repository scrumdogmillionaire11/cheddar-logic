---
name: pax:quick
description: Execute a quick task with PAX guarantees (atomic commits, state tracking) but skip optional agents
argument-hint: ""
allowed-tools:
  - Read
  - Write
...
<objective>
Execute small, ad-hoc tasks with PAX guarantees (atomic commits, STATE.md tracking) while skipping optional agents (research, plan-checker, verifier).

Quick mode is the same system with a shorter path:
- Spawns pax-planner (quick mode) + pax-executor(s)
- Skips pax-phase-researcher, pax-plan-checker, pax-verifier
...
<execution_context>
Orchestration is inline - no separate workflow file. Quick mode is deliberately simpler than full PAX.
</execution_context>
...
Check that an active PAX project exists:

```bash
if [ ! -f .planning/ROADMAP.md ]; then
  echo "Quick mode requires an active project with ROADMAP.md."
  echo "Run /pax:new-project first."
  exit 1
fi
```
...
Spawn pax-planner with quick mode context:
...
Spawn pax-executor with plan reference:

...
<objective>
Create all phases necessary to close gaps identified by `/pax:audit-milestone`.

Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase.

One command creates all fix phases — no manual `/pax:add-phase` per gap.
</objective>

<execution_context>
<!-- Spawns pax-planner agent which has all planning expertise baked in -->
</execution_context>
...
```
No audit gaps found. Run `/pax:audit-milestone` first.
```
...
**Also available:**
- `/pax:execute-phase {N}` — if plans already exist
...
`/pax:audit-milestone` — re-audit to verify gaps closed
`/pax:complete-milestone {version}` — archive when audit passes
```
...
- [ ] Changes committed
- [ ] User knows to run `/pax:plan-phase` next
</success_criteria>

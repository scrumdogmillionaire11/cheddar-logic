...
def _job(
    ...
    return SimpleNamespace(
        ...
        progress=progress,
        phase=phase,
        results=results or {"current_gw": 25},
        error=error,
...
def test_analyze_dashboard_contract(client, monkeypatch) -> None:
    monkeypatch.setattr(analyze_router.engine_service, "get_job", lambda _analysis_id: _job())
    response = client.get("/api/v1/analyze/test-analysis-id/dashboard")
    assert response.status_code == 200
    body = response.json()
    assert body["analysis_id"] == "test-analysis-id"
    assert body["status"] in {"complete", "completed"}


def test_user_analysis_history_contract(client, monkeypatch) -> None:
    monkeypatch.setattr(
...
def test_user_performance_contract(client, monkeypatch) -> None:
    monkeypatch.setattr(
        user_router.engine_service,

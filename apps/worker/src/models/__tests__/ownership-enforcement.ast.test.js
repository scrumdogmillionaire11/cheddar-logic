    ...
    jobFiles.forEach((filePath) => {
      ...
      Object.keys(FORBIDDEN).forEach((identifier) => {
        ...
        if (new RegExp(`function\\s+${identifier}\\s*\\(`).test(content)) {
          violations.push(
            `❌ ${identifier} defined in ${filePath} (should be in owner module)`,
          );
        }
        // Check for const/let/var assignment
        if (
          new RegExp(
            `const\\s+${identifier}\\s*=\\s*\\(.*?\\)|let\\s+${identifier}\\s*=\\s*\\(.*?\\)|var\\s+${identifier}\\s*=\\s*\\(.*?\\)`,
          ).test(content)
        ) {
          violations.push(
            `❌ ${identifier} assigned in ${filePath} (should be in owner module)`,
          );
        }
      });
    });

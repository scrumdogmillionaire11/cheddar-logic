...
function buildEdgeKey({ sport, gameId, market, side, line, book }) {
  if (!sport || !gameId || !market || !side || line === null || !book) return null;
  // Deterministic: sport|gameId|market|side|line|book (no timestamps, no UUIDs)
  return `${sport}|${gameId}|${market}|${side}|${line}|${book}`;
}

function formatEdgeFields({ sport, gameId, market, side, line, book, edgeResult }) {
...
function computeCardEdgeDecision({ sport, gameId, marketType, prediction, projectedMargin, projectedTotal, oddsSnapshot }) {
  if (!sport || !gameId || !marketType) return null;
  
  const decisions = {};
  
  if (marketType === 'TOTAL' && projectedTotal !== null && oddsSnapshot?.total_line !== null) {
    const totalEdge = edgeCalculator.computeTotalEdge({
      projectionTotal: projectedTotal,
    ...
    if (totalEdge) {
      decisions.TOTAL = formatEdgeFields({
        sport, gameId, market: 'TOTAL', side: 'OVER', 
  ...
  if ((marketType === 'SPREAD' || marketType === 'MONEYLINE') && projectedMargin !== null) {
    if (oddsSnapshot?.spread_line !== null) {
      ...
      if (spreadEdge) {
        decisions.SPREAD = formatEdgeFields({
          sport, gameId, market: 'SPREAD', side: prediction?.spreadSide || 'HOME',
    ...
    if (oddsSnapshot?.ml_price_home !== null && oddsSnapshot?.ml_price_away !== null) {
      ...
      if (mlEdge) {
        decisions.ML = formatEdgeFields({
          sport, gameId, market: 'MONEYLINE', side: prediction?.mlSide || 'HOME',
          line: null,
          book: getBookLabel(oddsSnapshot),
          edgeResult: mlEdge
        });

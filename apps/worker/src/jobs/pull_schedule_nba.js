 ...
 *
 * Usage:
 *   node src/jobs/pull_schedule_nba.js
...
async function pullScheduleNba({ jobKey = null, dryRun = false } = {}) {
  const jobRunId = `job-pull-schedule-nba-${new Date().toISOString().split('.')[0]}-${uuidV4().slice(0, 8)}`;

  console.log(`[Schedule:NBA] Starting job run: ${jobRunId}`);
  if (jobKey) console.log(`[Schedule:NBA] Job key: ${jobKey}`);
  console.log(`[Schedule:NBA] Time: ${new Date().toISOString()}`);
  ...
  return withDb(async () => {
    ...
    try {
      ...
      console.log(
        `[Schedule:NBA] Mapping: created=${mappingsCreated} no_candidate=${mappingsFailedNoCandidate} ambiguous=${mappingsFailedAmbiguous} skipped_canonical=${gamesSkippedCanonical}`,
      );
      return {
        success: true,
        jobRunId,
...
module.exports = { pullScheduleNba };

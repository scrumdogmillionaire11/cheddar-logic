...
async function pullScheduleNhl({ jobKey = null, dryRun = false } = {}) {
  ...
  return withDb(async () => {
    if (jobKey && !shouldRunJobKey(jobKey)) {
      console.log(`[Schedule:NHL] Skipping (already succeeded or running): ${jobKey}`);
      return { success: true, jobRunId: null, skipped: true, jobKey };
    }

    if (dryRun) {
      console.log('[Schedule:NHL] DRY_RUN=true -- would fetch ESPN scoreboards');
    ...
    try {
      ...
      console.log(`[Schedule:NHL] OK: upserted ${gamesUpserted} games across ${daysWithEvents}/${days.length} days`);
      console.log(`[Schedule:NHL] Mapping: created=${mappingsCreated} no_candidate=${mappingsFailedNoCandidate} ambiguous=${mappingsFailedAmbiguous}`);
      return {
        success: true,
        jobRunId,
        jobKey,
      ...
      try {
        ...
        console.error(`[Schedule:NHL] Failed to record error to DB:`, dbError.message);
      }

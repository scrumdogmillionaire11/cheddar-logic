...
const SPORTS = [
  ...
  },
  {
    ...
    teams: [
      ...
      'Purdue', 'Houston', 'Alabama', 'Tennessee', 'UCLA',
      'Arizona', 'Baylor', 'Creighton', 'UConn', 'Marquette',
      'Texas', 'Illinois', 'Auburn', 'Iowa State', 'San Diego State',
      'Saint Mary\'s', 'Michigan State', 'Wisconsin', 'Virginia', 'Florida Atlantic',
      'Arkansas', 'Northwestern', 'Memphis', 'TCU', 'Xavier',
      'Indiana', 'Maryland', 'Missouri', 'Texas A&M', 'Clemson',
      'Utah State', 'Colorado State', 'Drake', 'VCU', 'Dayton',
    ]
  }
];
...
async function run() {
  ...
  const sportFilter = process.argv.find(arg => arg.startsWith('--sport='))?.split('=')[1]?.toUpperCase();
  
  const nowEt = DateTime.now().setZone('America/New_York');
  const cacheDate = nowEt.toISODate();
  const jobKey = `refresh_team_metrics|${cacheDate}`;
  
  console.log(`[RefreshTeamMetrics] Starting run_id=${runId} (date=${cacheDate}, dry_run=${dryRun})`);
  ...
  try {
    ...
    for (const { sport, teams } of sportsToRefresh) {
      console.log(`[RefreshTeamMetrics] Processing ${sport} (${teams.length} teams)...`);
      ...
      for (const teamName of teams) {
        ...
        try {
          ...
          if (result.resolution?.status === 'ok') {
            ...
            failedCount++;
            failed.push({ sport, teamName, reason: result.resolution?.status || 'unknown' });
            console.warn(`  ✗ ${teamName} (reason=${result.resolution?.status || 'unknown'})`);
          }
    ...
    if (!dryRun) {
      ...
      console.log(`[RefreshTeamMetrics] Deleted ${deletedCount} stale cache entries (before ${staleDate})`);
    }
    ...
    console.log('[RefreshTeamMetrics] Summary:', JSON.stringify(summary, null, 2));
    
    if (!dryRun) {
      markJobRunSuccess(runId, summary);
    }
    
    console.log(`[RefreshTeamMetrics] Complete (success=${successCount}/${totalTeams})`);
    
    // Exit with error code if too many failures
    if (failedCount > totalTeams * 0.2) {
      console.error(`[RefreshTeamMetrics] WARN: High failure rate (${failedCount}/${totalTeams})`);
      process.exit(1);
    }

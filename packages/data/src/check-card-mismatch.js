...
async function checkMismatch() {
  ...
  let nbaCorrect = 0, nbaWrong = 0;
  nbaCards.forEach(c => {
    const isCorrect = c.card_type.toLowerCase().includes('nba');
    if (isCorrect) nbaCorrect++; else nbaWrong++;
  ...
  const allMismatches = db.prepare(`
  ...
  allMismatches.forEach(row => {
    if (!sportGroups[row.sport]) sportGroups[row.sport] = { correct: 0, wrong: 0, types: [] };
    ...
    sportGroups[row.sport].types.push({ type: row.card_type, count: row.count, match: isMatch });
    if (isMatch) {
